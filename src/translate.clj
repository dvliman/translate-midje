...
(defn translate-contains-arrow [[f1 f2 f3]]
  ...
  (let [[_ contents & options] f3]
    (match [f1 f2 contents]
      ...
      #_[actual '=> (_ :guard regex?)] #_`(~'is (~'re-find ~contents ~actual))
      [actual '=not=> (_ :guard string?)] `(~'is-not (~'re-find ~(re contents) ~actual))
      #_[actual '=not=> (_ :guard regex?)] #_`(~'is-not (~'re-find ~contents ~actual))
...
(defn translate-arrow [[f1 f2 f3 :as f]]
  ...
  (let [contains-form? (or (and (seq? f3) (= 'contains (first f3)))
        ...
        thrown? (and (list? f3)
                     (= 'throws (first f3)))]
    (cond
     #_(regex? f3) #_(translate-regex-arrow f)
...
#_(set-public! #'pprint/*code-table*)
#_(set-public! #'pprint/pprint-defn)
#_(set-public! #'pprint/pprint-let)
#_(set-public! #'pprint/pprint-hold-first)
